...
Step 1: 应用层变更**不依赖**基础设施层吗？
- 检查：去掉Docker配置变更，应用层代码是否可以独立运行？
- 示例：Java添加`logCommand`参数，可以独立运行和测试 ✅
- 示例：Python修改业务逻辑，可以独立运行和测试 ✅
- 如果是 → 倾向于拆分

Step 2: 基础设施层变更**不依赖**应用层吗？
- 检查：去掉应用层变更，基础设施层是否可以独立存在？
- 示例：Dockerfile移除healthcheck，docker-compose添加healthcheck，可以独立 ✅
- 示例：CI配置修改构建流程，可以独立 ✅
- 如果是 → 倾向于拆分

Step 3: 两者**仅有业务关联**吗？
- 检查：是否"同一个重构的不同部分"但技术上可以独立？
- 示例："重构healthcheck"但Java和Docker变更独立 ✅
- 示例："优化日志"但代码和配置独立 ✅
- 如果是 → **应该拆分**（注意："同一重构"不等于"有依赖"）

Step 4: 检查**明确的代码依赖**？
- 代码是否使用了新配置？（如`@Value`, `getenv()`）
- 是否有API/Schema变更需要同步？
- 是否有重命名需要所有使用方同步？
- 如果都不是 → **无依赖，应该拆分**

**关键原则**: 应用层+基础设施层，**默认倾向于拆分**，除非有明确的代码依赖证据。

⚠️ **常见误判**:
- ❌ "Java支持Docker配置" → 误判为有依赖（实际是业务关联，应拆分）
- ❌ "同一重构的不同部分" → 误判为应合并（实际可以独立，应拆分）
- ❌ "功能相关" → 误判为有依赖（需要明确的代码依赖证据）
...
当满足以下任一条件时，**MUST use AskUserQuestion**询问用户：
...
- 这是**最常见的误判场景**
- 必须严格按照上面的4步判定流程检查
- 如果判定为"应该拆分"但又有疑虑，**立即询问用户**
- **不要**因为"同一重构"、"功能相关"、"支持"等理由就跳过判定流程

**关键原则**:
1. 应用层+基础设施层，**默认倾向于拆分**
2. 必须找到**明确的代码依赖证据**才能合并
3. 如果不确定，**宁可询问用户**，也不要错误合并
4. "业务关联" ≠ "代码依赖"

**参考案例**: See [references/DEPENDENCY_CRITERIA.md#失败案例分析](references/DEPENDENCY_CRITERIA.md#失败案例分析)
