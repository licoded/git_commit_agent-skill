...
应用层+基础设施层的混合变更**是最常见的误判场景**。必须严格按照以下4步流程进行判定：

### Step 1: 应用层变更不依赖基础设施层吗？

**检查问题**：去掉基础设施层变更，应用层代码是否可以独立运行和测试？

**示例**:
- ✅ Java添加`logCommand`参数 → 可以独立运行和测试
- ✅ Python修改业务逻辑 → 可以独立运行和测试
- ✅ TypeScript添加新函数 → 可以独立运行和测试

**判定**:
- 如果YES → 倾向于拆分
- 如果NO（代码依赖Docker配置才能运行）→ 继续Step 2-4检查

### Step 2: 基础设施层变更不依赖应用层吗？

**检查问题**：去掉应用层变更，基础设施层是否可以独立存在？

**示例**:
- ✅ Dockerfile移除healthcheck，docker-compose添加healthcheck → 可以独立存在
- ✅ CI配置修改构建流程 → 可以独立存在
- ✅ .dockerignore添加Dockerfile → 可以独立存在

**判定**:
- 如果YES → 倾向于拆分
- 如果NO（Docker配置依赖应用代码变更）→ 继续Step 3-4检查

### Step 3: 两者仅有业务关联吗？

**检查问题**：是否"同一个重构的不同部分"但技术上可以独立？

**关键理解**：
- ❌ **错误理解**："同一重构" = "有依赖" = 应合并
- ✅ **正确理解**："同一重构"但"技术上独立" = 无依赖 = 应拆分

**示例**:
- ✅ "重构healthcheck"但Java和Docker变更独立 → 应拆分
- ✅ "优化日志"但代码和配置独立 → 应拆分
- ✅ "统一配置"但应用层和基础设施层独立 → 应拆分

**判定**:
- 如果是"同一重构"但技术上可以独立 → **应该拆分**
- 如果不是同一重构 → 继续Step 4检查

### Step 4: 检查明确的代码依赖证据？

**检查清单**（必须找到明确的代码依赖证据才能合并）：

1. **代码是否使用了新配置？**
   - Java: `@Value`, `@Property`, `@ConfigurationProperties`, `@Autowired`
   - Python: `os.getenv()`, `os.environ`, `config.get()`
   - JavaScript: `process.env`, `config.get()`
   - 如果是 → 有依赖，应合并

2. **是否有API/Schema变更需要同步？**
   - OpenAPI/GraphQL schema变更
   - Protobuf定义变更
   - 接口签名变更
   - 如果是 → 有依赖，应合并
...
1. **默认倾向于拆分**：应用层+基础设施层，默认拆分
2. **举证责任在"合并"**：必须找到明确的代码依赖证据才能合并
3. **业务关联 ≠ 代码依赖**：功能相关不代表有技术依赖
4. **不确定时询问用户**：如果判定为"应该拆分"但又有疑虑，立即使用AskUserQuestion

### 常见误判警示

⚠️ **以下说法都是错误的误判信号**：
- ❌ "Java代码支持Docker配置" → 误判为有依赖
- ❌ "同一重构的不同部分" → 误判为应合并
- ❌ "功能相关" → 误判为有依赖
- ❌ "这是协调的重构" → 误判为应合并
